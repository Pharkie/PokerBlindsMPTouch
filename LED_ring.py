...
async def flash_all(color, duration):
    end_time = time.time() + duration
    step = 50  # Larger step size for brightness change
...
async def startup_effect():
    ...
    for cycle in range(3):  # Run 3 cycles
        for i in range(NUM_LEDS):
            ...
            await asyncio.sleep(ANIMATION_WAIT)
        # Fade to new color
        for brightness in range(255, -1, -20):  # Larger step size
            for i in range(NUM_LEDS):
...
async def main():
    await startup_effect()  # Play startup effect once
    while True:
        for _ in range(3):  # Complete 3 circles
            await spin_with_trail()
        await flash_all(FLASH_COLOR, FLASH_DURATION)  # Flash all LEDs
