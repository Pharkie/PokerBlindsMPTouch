...
modestr = {
    ...
    GS8: "8-bit color RRRGGGBB",
    GS4_HMSB: "4-bit greyscale",
}
...
def convgs(arr, rows, cols, si, so):
    ...
    for row in range(rows):
        for col in range(cols):
            ra[row][col] = (
                int.from_bytes(si.read(1), "big") << 8
            )  # 16 bit greyscale
    for row in range(rows):
        col = 0
        while col < cols:
            for n in range(2):
                ...
                e = c & 0x0FFF  # error
                dither(
                    arr, ra, row, col, rows, cols, e
                )  # Adjust forward pixels
                col += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "outfile", type=str, help="Path and name of output file"
    )
    parser.add_argument(
        "-r", "--rows", type=int, help="Image height (rows) in pixels"
    )
    parser.add_argument(
        "-c", "--cols", type=int, help="Image width (cols) in pixels"
    )
    parser.add_argument(
        "-d",
        "--dither",
    ...
    parser.add_argument(
        "--rgb565", action="store_true", help="Create 16-bit RGB565 file."
    )
